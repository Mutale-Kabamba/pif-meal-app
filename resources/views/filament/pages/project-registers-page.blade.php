    ...
    <div style="display: flex; flex-direction: column; gap: 1rem;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 0.75rem; flex-wrap: wrap; background: #ffffff; padding: 0.85rem 1.25rem; border-radius: 1rem; border: 1px solid #e5e7eb;" class="dark:!bg-gray-900 dark:!border-gray-800">
            <div style="display: flex; align-items: center; gap: 0.65rem;">
                <div style="width: 2.25rem; height: 2.25rem; border-radius: 0.65rem; background: #ecfdf5; display: flex; align-items: center; justify-content: center; color: #047857;" class="dark:!bg-emerald-950 dark:!text-emerald-300">
                ...
                <div>
                    <h2 style="font-size: 0.95rem; font-weight: 800; color: #0f172a; margin: 0; line-height: 1.2;" class="dark:!text-white">
                        Attendance Register (Training / Classes)
                    </h2>
                    <p style="font-size: 0.75rem; color: #64748b; margin: 0; margin-top: 2px;">
                        Training session attendance marked by Coaches &amp; Literacy session attendance marked by Project Officers
                    </p>
                </div>
            </div>
        ...
        {{-- Role / Operational Notice for Admins --}}
        @if(!$canMark)
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.6rem 1rem; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 0.75rem; font-size: 0.75rem; color: #166534;" class="dark:!bg-emerald-950/40 dark:!border-emerald-900 dark:!text-emerald-300">
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    <span><strong>Administrator Monitoring View:</strong> Register marking is reserved for Coaches and Project Officers. System managers and administrators have read-only audit and PDF export capabilities.</span>
                </div>
        ...
        <div style="padding: 1rem; background: #ffffff; border-radius: 1rem; border: 1px solid #e5e7eb;" class="dark:!bg-gray-900 dark:!border-gray-800">
            ...
            <div style="margin-top: 0.85rem; padding-top: 0.75rem; border-top: 1px solid #f3f4f6; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.5rem;" class="dark:!border-gray-800">
                <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.75rem;">
                    <span style="font-weight: 700; color: #111827;" class="dark:!text-white">Active Register:</span>
                    <span style="display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.2rem 0.55rem; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 0.4rem; color: #166534; font-weight: 600;" class="dark:!bg-emerald-950/30 dark:!border-emerald-900 dark:!text-emerald-300">
                        {{ $registerLabel }}
                    </span>
                    <span style="color: #6b7280;">&bull;</span>
                    <span style="color: #4b5563;" class="dark:!text-gray-300">
                        Activity: <strong>{{ $activityLabel }}</strong>
                    </span>
                    <span style="color: #6b7280;">&bull;</span>
                                    ...
                                    @foreach($weeksStructure as $weekNum => $days)
                                        @foreach($days as $dayMeta)
                                            @php 
                                                 ...
                                                 $status = $attendanceMatrix[$beneficiary->id][$dayNum] ?? null;
                                                 if ($status === 'present' || $status === 'late') $totalCount++;
                                            @endphp
                                            <td style="padding: 0.15rem 0.1rem; border-right: 1px solid #f1f5f9; text-align: center; user-select: none; {{ $canMark ? 'cursor: pointer;' : 'cursor: default;' }}" 
                                                class="dark:!border-gray-800"
                                                @if($canMark) wire:click="toggleAttendance({{ $beneficiary->id }}, {{ $dayNum }})" title="Click to toggle attendance" @endif>
                                                @if($status === 'present')
